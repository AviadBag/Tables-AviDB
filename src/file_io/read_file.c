...
void free_column(struct s_column column)
{
    free(column.name);
}

void free_columns_structure(struct s_columns_structure table_columns_structure)
{
    for (int i = 0; i < table_columns_structure.length; i++)
        free_column(table_columns_structure.columns[i]);
}

void free_table_header(struct s_table_header table_header)
{
    free(table_header.table_name);
    free_columns_structure(table_header.columns_structure);
}

void free_table_body(struct s_table_body table_body)
{
...
{
    free_table_header(table.table_header);
    free_table_body(table.table_body);
}
