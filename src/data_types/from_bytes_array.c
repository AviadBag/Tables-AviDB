 ...
 */
static struct s_data_holder from_string_bytes_array(byte* bytes_array, int length)
{
 ...
 * @return A data_holder contains the data.
 */
static struct s_data_holder from_integer_bytes_array(byte* bytes_array, int length)
{
    int integer = 0;
 ...
 */
struct s_data_holder from_bytes_array(enum e_type type, byte* bytes_array, int length)
{
    switch (type)
    {
        case INTEGER:
            return from_integer_bytes_array(bytes_array, length);
        
        case STRING:
            return from_string_bytes_array(bytes_array, length);
    }
